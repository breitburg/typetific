...
class TypingWindow(QMainWindow):
    ...
    def paintEvent(self, event):
        ...
        self.painter.setFont(QFont('Menlo', max(32, event.rect().height() / self.settings.value('fontFactor', 6))))
        ...
        lineWidth = letterSize.maxWidth() + self.settings.value('linePadding', 10)
        linePadding = event.rect().width() / 4
        lineX = linePadding if not self.settings.value('enableSliding', False) else linePadding + (
        ...
        if not self.isEnd:
            ...
            self.painter.drawText(lineX + self.settings.value('linePadding', 10) / 2, textY, self.text[self.pointer])
            self.painter.drawText(lineX + lineWidth + self.settings.value('linePadding', 10) / 2, textY,
        ...
        self.painter.drawText(lineX - typedTextSize.width() - self.settings.value('linePadding', 10) / 2, textY,
    ...
    def keyPressEvent(self, event) -> None:
        if event.key() == Qt.Key_Return:
            self.reset()
        elif self.settings.value('enableBackspace', True) and event.key() == Qt.Key_Backspace and self.pointer > 0:
            self.pointer -= 1
    ...
    def reset(self) -> None:
        self.events = []
        self.text = ' '.join(
            [choice(list(self.config['dictionary'])).lower() for _ in range(self.settings.value('wordsCount', 20))])
        self.pointer = 0
